...
int main(int argc, char **argv)	{
	...
	printf("[+] Keyhunt Revived\n");
	printf("[+] Version %s, developed by AlbertoBSD\n",version);
	printf("[+] forked and optimized by Draikoon\n");
	printf("[+] AVX2 acceleration: %s\n", keyhunt_avx2_available() ? "enabled" : "not available; using fallback");
		...
		switch(c) {
			...
			case 'C':
				if(strlen(optarg) == 22)	{
					...
					memcpy(str_baseminikey,optarg,22);
...
void *thread_process(void *vargp)	{
	...
	char compressed_hash_cache[4][20];
	
	/*
	 * Compressed keys need the Y parity to select their 02/03 prefix when
	 * endomorphism is disabled.  The optimized hash path emits only the
	 * prefix belonging to the actual point, so an X-only point is not enough.
	 */
	bool calculate_y = FLAGSEARCH == SEARCH_UNCOMPRESS || FLAGSEARCH == SEARCH_BOTH ||
		FLAGCRYPTO == CRYPTO_ETH ||
		(FLAGCRYPTO == CRYPTO_BTC && FLAGSEARCH == SEARCH_COMPRESS && !FLAGENDOMORPHISM);
									...
									if(FLAGENDOMORPHISM)	{
										...
										*/
										...
										if((j & 1U) == 0) {
											...
											for(k = 0; k < 8; ++k) {
												x[k] = &pts[(j*4)+k].x;
												yOdd[k] = pts[(j*4)+k].y.IsOdd();
												hash[k] = k < 4
													? (uint8_t*)publickeyhashrmd160_endomorphism[0][k]
													: (uint8_t*)compressed_hash_cache[k-4];
											}
											secp->GetCompressedHash160AVX2(x,yOdd,hash);
										}
										else {
											for(k = 0; k < 4; ++k)
												memcpy(publickeyhashrmd160_endomorphism[0][k],compressed_hash_cache[k],20);
										}
								...
								for(k = 0; k < 4;k++)	{
									if(FLAGSEARCH == SEARCH_COMPRESS || FLAGSEARCH == SEARCH_BOTH){
										if(FLAGENDOMORPHISM)	{
											...
											/* The non-endomorphism path emits one hash with the point's actual prefix. */
											for(l = 0;l < 1; l++)	{
												r = bloom_check(&bloom,publickeyhashrmd160_endomorphism[l][k],MAXLENGTHADDRESS);
												if(r) {
													r = searchbinary(addressTable,publickeyhashrmd160_endomorphism[l][k],N);
