...
#ifdef _MSC_VER
#include <malloc.h>
#define STACK_ALLOC(type, name, count) type *name = (type *)_alloca(sizeof(type) * (count))
#else
#define STACK_ALLOC(type, name, count) type name[count]
#endif
...
bool b58tobin(void *bin, size_t *binszp, const char *b58, size_t b58sz)
{
	...
	STACK_ALLOC(b58_almostmaxint_t, outi, outisz);
	b58_maxint_t t;
...
{
	STACK_ALLOC(uint8_t, buf, 1 + datasz + 0x20);
