...
// Process eight independent, already padded SHA-256 blocks in parallel.
// This is the natural shape for keyhunt: every SIMD lane contains a different
// compressed public key.  A runtime fallback preserves compatibility with
// processors that do not provide AVX2.
...
#define AVX2_CH(x,y,z) _mm256_xor_si256(_mm256_and_si256((x),(y)), _mm256_andnot_si256((x),(z)))
#define AVX2_MAJ(x,y,z) _mm256_or_si256(_mm256_and_si256((x),(y)), _mm256_and_si256((z),_mm256_or_si256((x),(y))))

#if defined(__GNUC__) || defined(__clang__)
__attribute__((target("avx2")))
#endif
static void transform(uint32_t *input[8], uint8_t *output[8]) {
  __m256i w[64];
  for (int i = 0; i < 16; ++i) {
    w[i] = _mm256_setr_epi32(input[0][i], input[1][i], input[2][i], input[3][i],
                             input[4][i], input[5][i], input[6][i], input[7][i]);
  }
  for (int i = 16; i < 64; ++i) {
    const __m256i s0 = _mm256_xor_si256(
      AVX2_ROR(w[i - 15], 7),
      _mm256_xor_si256(AVX2_ROR(w[i - 15], 18), _mm256_srli_epi32(w[i - 15], 3)));
    const __m256i s1 = _mm256_xor_si256(
      AVX2_ROR(w[i - 2], 17),
      _mm256_xor_si256(AVX2_ROR(w[i - 2], 19), _mm256_srli_epi32(w[i - 2], 10)));
    w[i] = _mm256_add_epi32(_mm256_add_epi32(w[i - 16], s0),
                            _mm256_add_epi32(w[i - 7], s1));
  }

  const __m256i ia = _mm256_set1_epi32(0x6A09E667);
  const __m256i ib = _mm256_set1_epi32(0xBB67AE85);
  const __m256i ic = _mm256_set1_epi32(0x3C6EF372);
  const __m256i id = _mm256_set1_epi32(0xA54FF53A);
  const __m256i ie = _mm256_set1_epi32(0x510E527F);
  const __m256i iff = _mm256_set1_epi32(0x9B05688C);
  const __m256i ig = _mm256_set1_epi32(0x1F83D9AB);
  const __m256i ih = _mm256_set1_epi32(0x5BE0CD19);
  __m256i a = ia, b = ib, c = ic, d = id, e = ie, f = iff, g = ig, h = ih;

  for (int i = 0; i < 64; ++i) {
    const __m256i sum1 = _mm256_xor_si256(
      AVX2_ROR(e, 6), _mm256_xor_si256(AVX2_ROR(e, 11), AVX2_ROR(e, 25)));
    const __m256i sum0 = _mm256_xor_si256(
      AVX2_ROR(a, 2), _mm256_xor_si256(AVX2_ROR(a, 13), AVX2_ROR(a, 22)));
    __m256i t1 = _mm256_add_epi32(h, sum1);
    t1 = _mm256_add_epi32(t1, AVX2_CH(e, f, g));
    t1 = _mm256_add_epi32(t1, _mm256_set1_epi32(k[i]));
    t1 = _mm256_add_epi32(t1, w[i]);
    const __m256i t2 = _mm256_add_epi32(sum0, AVX2_MAJ(a, b, c));
    h = g; g = f; f = e; e = _mm256_add_epi32(d, t1);
    d = c; c = b; b = a; a = _mm256_add_epi32(t1, t2);
  }

  __m256i state[8] = {
    _mm256_add_epi32(a, ia), _mm256_add_epi32(b, ib),
    _mm256_add_epi32(c, ic), _mm256_add_epi32(d, id),
    _mm256_add_epi32(e, ie), _mm256_add_epi32(f, iff),
    _mm256_add_epi32(g, ig), _mm256_add_epi32(h, ih)
  };
  uint32_t words[8][8];
  for (int i = 0; i < 8; ++i)
    _mm256_storeu_si256((__m256i *)words[i], state[i]);
  for (int lane = 0; lane < 8; ++lane) {
    for (int word = 0; word < 8; ++word) {
      const uint32_t value = words[word][lane];
      output[lane][4 * word + 0] = (uint8_t)(value >> 24);
      output[lane][4 * word + 1] = (uint8_t)(value >> 16);
      output[lane][4 * word + 2] = (uint8_t)(value >> 8);
      output[lane][4 * word + 3] = (uint8_t)value;
    }
  }
}
...
void sha256avx2_1B_impl(
  uint32_t *i0, uint32_t *i1, uint32_t *i2, uint32_t *i3,
  uint32_t *i4, uint32_t *i5, uint32_t *i6, uint32_t *i7,
  uint8_t *d0, uint8_t *d1, uint8_t *d2, uint8_t *d3,
  uint8_t *d4, uint8_t *d5, uint8_t *d6, uint8_t *d7) {
  uint32_t *input[8] = {i0,i1,i2,i3,i4,i5,i6,i7};
  uint8_t *output[8] = {d0,d1,d2,d3,d4,d5,d6,d7};
  _sha256avx2::transform(input, output);
}
...
void sha256avx2_1B(
  ...
  if (keyhunt_avx2_available()) {
    sha256avx2_1B_impl(i0,i1,i2,i3,i4,i5,i6,i7,d0,d1,d2,d3,d4,d5,d6,d7);
  } else {
