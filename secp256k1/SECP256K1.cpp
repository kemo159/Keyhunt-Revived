...
Point Secp256K1::ComputePublicKey(Int *privKey) {
  ...
  // Valid secp256k1 private keys are in [1, order - 1].  Without this
  // guard, a zero key indexes GTable before its first element.
  if (privKey == NULL || privKey->IsZero() || privKey->IsGreaterOrEqual(&order))
    return Q;
...
#define KEYBUFFCOMPQ(buff,px,odd) \
...
void Secp256K1::GetCompressedHash160(int type,
  ...
  case BECH32:
  {
    ...
    KEYBUFFCOMPQ(b0, x0, y0Odd);
    KEYBUFFCOMPQ(b1, x1, y1Odd);
    KEYBUFFCOMPQ(b2, x2, y2Odd);
    KEYBUFFCOMPQ(b3, x3, y3Odd);
...
void Secp256K1::GetCompressedHash160AVX2(Int **x, const bool *yOdd, uint8_t **hash) {
  uint32_t block[8][16];
  alignas(32) uint8_t sha[8][64];

  for (int i = 0; i < 8; ++i) {
    KEYBUFFCOMPQ(block[i], x[i], yOdd[i]);
  }
  ...
  uint8_t *shaInput[8] = {sha[0],sha[1],sha[2],sha[3],sha[4],sha[5],sha[6],sha[7]};
  ripemd160avx2_32(shaInput,hash);
}
